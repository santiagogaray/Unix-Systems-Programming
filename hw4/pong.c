 ...
 *
 *	user input:
 *		 	k moves the paddle up
 *		 	m moved the paddle down
 *			Q quits from game
 *
 */
...
/*	
 *	set_up()
 *		Initializes drawing mode.
		Initialixes the ball and paddle states.
 *		Displays the court.
 *		Displays the balls left in game
 *
 */
set_up()
{
	char szballs_left[32];	/* text used to display balls left */
...
/*
 *	paddle_move()
 *		Checks if a paddle key was pressed,
 *		draws the paddle in new position
...
/*
 *	ball_move()
 *		Checks if the ball position changes in
 *		current signal call , updates
 *		states and displays ball.
 *		return:	int 1	indicates the ball moved.
 *				int 0	indicates the ball's position
 *						didn't change.
 */
ball_move()
{
 ...
 *		the paddle is present.
 *		return:	int 1	if the ball is out of play
 *				int	0	if the ball is on play
 */
